...
const ChatBubblev2 = (props) => {
  ...
  const [feedbacks, setFeedbacks] = useState(Array(messages?.length).fill(0));
    ...
    useEffect(() => {
      let isKeyPressed = new Set(); // Use a Set to track pressed keys
    
      const getCurrentTimestamp = () => {
        const now = new Date();
        return now.toISOString(); // Return ISO string format of the current time
      };
    
      const handleKeyDown = (e) => {
        // Check if the key is "T" or "t" and is not already pressed
        if ((e.key === "T" || e.key === "t") && !isKeyPressed.has(e.key)) {
          isKeyPressed.add(e.key); // Add key to the set of pressed keys
          setEnter(true); // Update the state for key press
          console.log("Key holding started:", { timestamp: getCurrentTimestamp() });
        }
      };
    
      const handleKeyUp = (e) => {
        // Check if the key is "T" or "t" and is currently pressed
        if ((e.key === "T" || e.key === "t") && isKeyPressed.has(e.key)) {
          isKeyPressed.delete(e.key); // Remove key from the set of pressed keys
          setEnter(false); // Update the state for key release
          console.log("Key released:", { timestamp: getCurrentTimestamp() });
          console.log(userText); // Log the user input on key release
        }
      };
    ...
    }, [userText]); // Include userText as a dependency to log updated user input
